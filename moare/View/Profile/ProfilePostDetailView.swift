...
struct ProfilePostDetailView: View {
    ...
    @State var updateViewPresented = false
        ...
        ZStack {
            ...
            VStack() {
                ...
                HStack {
                    ...
                    HStack {
                        if post.like!.contains(profileVM.username) {
...
//            }
            ...
            ToolbarItem(placement: .navigationBarTrailing) {
                Menu {
                    Button(StringResources.update) { updateViewPresented = true }
                    Button(StringResources.delete) {
                        profileVM.deletePost(post: post, listIndex: listIndex, postIndex: postIndex)
                    }
                } label: {
                    RoundedRectangle(cornerRadius: 10)
                        .fill(.black)
        ...
        .fullScreenCover(isPresented: $updateViewPresented, onDismiss: {}, content: {
            PostUpdateView(post: post, listIndex: listIndex, postIndex: postIndex)
        })
    }
}
